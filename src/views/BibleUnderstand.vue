<template>
  <div class="bible-understand">
    <v-container>
      <v-row>
        <v-col>
          <h1 class="text-center my-10">성서 이해</h1>
          <p class="text-center mb-12">성서이해는 성경을 처음 접하는 분들이 쉽게 이해하고 공부할 수 있도록 다양한 자료와 해설을 제공합니다.</p>
        </v-col>
      </v-row>
      <v-row>
        <v-col>
...
export default {
  ...
  setup() {
    ...
    async function fetchStudyItems() {
      ...
      const q = query(
        collection(db, 'blogs'),
        where('category', '==', '성서이해'), // 'category' 필터 추가
