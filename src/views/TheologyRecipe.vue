<template>
  <div class="theology-recipe">
    <v-container>
      <v-row>
        <v-col>
          <h1 class="text-center my-10">신학 레시피</h1>
          <p class="text-center mb-12">신학레시피는 신학의 다양한 주제를 맛깔나게 풀어내어, 누구나 신학을 쉽게 이해하고 적용할 수 있도록 돕는 자료를 제공합니다.</p>
        </v-col>
      </v-row>
      <v-row>
        <v-col>
...
export default {
  ...
  setup() {
    ...
    async function fetchStudyItems() {
      ...
      const q = query(
        collection(db, 'blogs'),
        where('category', '==', '신학레시피'), // 'category' 필터 추가
