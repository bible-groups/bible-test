<template>
  <div class="word-meditation">
    <v-container>
      <v-row>
        <v-col>
          <h1 class="text-center my-10">말씀 새김</h1>
          <p class="text-center mb-12">말씀새김은 성경 말씀을 깊이 있게 묵상하고 삶에 적용할 수 있도록 다양한 묵상 자료와 가이드를 제공합니다.</p>
        </v-col>
      </v-row>
      <v-row>
        <v-col>
...
export default {
  ...
  setup() {
    ...
    async function fetchStudyItems() {
      ...
      const q = query(
        collection(db, 'blogs'),
        where('category', '==', '말씀새김'), // 'category' 필터 추가
