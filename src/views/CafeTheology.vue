<template>
  <div class="cafe-theology">
    <v-container>
      <v-row>
        <v-col>
          <h1 class="text-center my-10">성서 카페</h1>
          <p class="text-center mb-12">카페 신학은 일상 속에서 쉽게 접할 수 있는 신학 이야기를 제공하며, 신학에 대한 친근한 접근을 돕습니다.</p>
        </v-col>
      </v-row>
      <v-row>
        <v-col>
...
export default {
  ...
  setup() {
    ...
    async function fetchStudyItems() {
      ...
      const q = query(
        collection(db, 'blogs'),
        where('category', '==', '카페신학'), // 'category' 필터 추가
